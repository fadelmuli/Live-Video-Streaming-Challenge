...
def test(user_id):
    ...
    NETWORK_TRACE = ['fixed', 'low', 'medium', 'high']

    # Change which set of video trace to use.
    VIDEO_TRACE = ['AsianCup_China_Uzbekistan', 'Fengtimo_2018_11_3', 'game', 'room', 'sports']
    ...
    for network in NETWORK_TRACE:
        ...
        for video in VIDEO_TRACE:
            ...
            while True:
                ...
                if not cdn_flag:
                    bu = frame_time_len * float(BIT_RATE[bit_rate]) / 1000
                    rp = REBUF_PENALTY * rebuf
                    lp = LANTENCY_PENALTY  * end_delay
                    sf = SKIP_PENALTY * skip_frame_time_len
                    reward_frame = bu  - rp - lp - sf
                ...
                if decision_flag or end_of_video:
                    ...
                    sp = SMOOTH_PENALTY * (abs(BIT_RATE[bit_rate] - BIT_RATE[last_bit_rate]) / 1000)
                    reward_frame += -1 * sp
                    sp_frame += sp
                ...
                if end_of_video:
                    ...
                    if trace_count >= len(all_file_names):
                        print("epoch total reward: %f" % (reward_all_sum / trace_count))
                        print("bitrate util: %f" % (bu_all_sum / trace_count))
                        print("rebuff penalty: %f" % (rp_all_sum / trace_countt))
                        print("latency penalty: %f" % (lp_all_sum / trace_count)) 
                        print("smooth penalty: %f" % (sp_sum / trace_count))
                        print("skip penalty: %f" % (sf_sum / trace_count))
                        break
                    trace_count += 1
                    cnt = 0
